...
def _gazebo_resource_path(context):
    ...
    bundled_paths = [
        ...
        gazebo_share,
        os.path.dirname(get_package_share_directory("rlai_meshes")),
    ]
    existing_paths = _path_entries(os.environ.get("GZ_SIM_RESOURCE_PATH", ""))
    extra_paths = _path_entries(
        LaunchConfiguration("extra_gz_resource_path").perform(context)
...
def _gazebo_plugin_path():
    existing_paths = _path_entries(os.environ.get("GZ_SIM_SYSTEM_PLUGIN_PATH", ""))
    default_paths = ["/opt/ros/jazzy/lib"]
    return os.pathsep.join(
        _unique_paths(default_paths + existing_paths)
    )
